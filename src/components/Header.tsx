          ...
          <nav className="hidden md:flex space-x-8">
            {["About", "Experience", "Projects", "Skills", "Contact"].map(item => (
              <button
                ...
                className="text-cyan-100 hover:text-cyan-400 transition-colors duration-300 relative group font-medium"
              >
                {item.toUpperCase()}
          ...
          <nav className="md:hidden mt-4 pb-4 space-y-2 border-t border-cyan-500/20 pt-4">
            {["About", "Experience", "Projects", "Skills", "Contact"].map(item => (
              <button
                key={item}
                onClick={() => scrollToSection(item.toLowerCase())}
                className="block w-full text-left text-cyan-100 hover:text-cyan-400 transition-colors duration-300 py-2 font-medium"
              >
                {item.toUpperCase()}
              </button>
            ))}
